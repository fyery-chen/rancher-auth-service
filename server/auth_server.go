...
func Reload(fromUpdate bool) (bool, error) {
	...
	case *refreshReqChannel <- 1:
		...
		provider = newProvider
		authConfigInMemory = authConfig
		//if success, then send heartbeat to Yunhong platform
		if yunhong.Ticker != nil {
			yunhong.Ticker.Stop()
		}
		if authConfig.Provider == "yunhongconfig" {
			provider.(*yunhong.YHProvider).SendHeartbeatRequest(authConfig)
		}
		<-*refreshReqChannel
		return false, nil
	default:
...
func InitCASAuth(w http.ResponseWriter, r *http.Request, t string, isTest string) (bool, string) {
	if provider != nil {
		return provider.(*yunhong.YHProvider).ProcessCasAuth(w, r, t, isTest)
	}
	return false, ""
}
